...
static int init_redis()
{
	...
	speaker.cache_rc = new RedisClient();
	ret = speaker.cache_rc->init(speaker.conf["cache-db"]["host"].asString(),
			speaker.conf["cache-db"]["port"].asInt(), 1000, speaker.conf["cache-db"]["pass"].asString());
	if (ret < 0) {
