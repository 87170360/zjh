...
{
    public:
        betInfo(int _uid, int _bet_num)
            : uid(_uid), bet_num(_bet_num)
        {}
    int uid;
    int bet_num;
};
...
{
	public:
		historyInfo(int _uid, double _money, int _card, int _bet_num, std::string _name)
            : uid(_uid), money(_money), card(_card), bet_num(_bet_num), name(_name)
		{}
...
private:
    ...
    std::deque<historyInfo> m_historyInfo;
...
private:
    ...
    void updateChangeValue(double value);
