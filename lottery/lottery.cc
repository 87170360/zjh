...
int Lottery::start_bet()
{
    ...
    int idx = 0;
    for(std::deque<historyInfo>::iterator iter = m_historyInfo.begin(); iter != m_historyInfo.end(); ++iter)
    {
        //xt_log.info("history, uid:%d, money:%.0f, card:%d, name:%s, bet_num:%d\n", iter->uid, iter->money, iter->card, iter->name.c_str(), iter->bet_num); 
	    packet.val["history"][idx]["name"] = iter->name;
	    packet.val["history"][idx]["card"] = iter->card;
	    packet.val["history"][idx]["money"] = iter->money;
	    packet.val["history"][idx]["bet_num"] = iter->bet_num;
        idx++;
    }
...
int Lottery::handler_betting(Client *client, int type,int bet_nu)
{
    ...
    if (bet_nu < 0)
    {
		xt_log.error("handle betting, bet_nu < 0 [%d].\n", bet_nu);
		return -1;
    }
	...
	if (type == card_type) 
	{
		...
		xt_log.info("lottery betting win uid[%d] type[%d] card_type[%d].\n", _uid, type, card_type);

	}
   	else 
	{
		xt_log.info("lottery betting lose uid[%d] type[%d] card_type[%d].\n", _uid, type, card_type);
	}
...
int Lottery::handler_awards()
{
    ...
    double single_fee = single_money / 0.95f;
...
//		send_awards_unicast(_uid, value);
		updateChangeValue(single_fee * it->second);
	}
    saveHistory();
...
bool compareHistory(const betInfo& l, const betInfo& r) 
{
    return l.bet_num > r.bet_num;
}
    
void Lottery::saveHistory(void)
{
    std::vector<betInfo> tmpVec;
	for(std::map<int,int>::iterator iter = awards_client.begin(); iter != awards_client.end(); ++iter) 
    {
        tmpVec.push_back(betInfo(iter->first, iter->second));
    }

    std::sort(tmpVec.begin(), tmpVec.end(), compareHistory);
    ...
    int tuid = tmpVec.begin()->uid;
    int tbet_num = tmpVec.begin()->bet_num;
    if(action_client.find(tuid) == action_client.end())
    {
        return;
    }
    std::string tname = action_client[tuid]->name;

    m_historyInfo.push_back(historyInfo(tuid, tbet_num * single_money, card_type, tbet_num, tname));

	if(m_historyInfo.size() > (unsigned int)10)
	{
		m_historyInfo.pop_front();
	}
}
    
void Lottery::updateChangeValue(double value)
{
    xt_log.info("updateChangeValue, value:%.0f\n", value);
	int ret = speaker.cache_rc->command("hincrby ak_info m_change_value %.0f", value);
	if(ret < 0)
	{
		xt_log.error("cache redis error( hincrby ak_info m_change_value).\n");
	}
} 
