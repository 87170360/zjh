...
void XtBeautyGame::updateChangeValue(double value)
{
    dynamic_cast<XtBeautyServer*>(m_server)->updateChangeValue(value);
}
...
void XtBeautyGame::handlePlayerBetResult()
{
	...
	m_gameResultInfo.push_back(XtBeautyGameResult(m_gameBetting.m_leftWin,m_gameBetting.m_middleWin,!m_gameBetting.m_leftWin));
...
void XtBeautyGame::sendGameInfo(XtBeautyPlayer* player)
{
	...
	int i=0;
	for(std::deque<XtBeautyGameResult>::iterator iter=m_gameResultInfo.begin();
			iter!=m_gameResultInfo.end();
			++iter)
	{
		int j=0;
