...
void XtBeautyServer::updateChangeValue(double value)
{
	int ret = m_cacheRc->doCommand("hincrby ak_info m_change_value %.0f \n", value);
	if(ret < 0)
	{
		XT_LOG_INFO("cache redis error( hincrby ak_info m_change_value).\n");
	}
}
